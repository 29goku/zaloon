...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: salon ? `Leave a Review — ${salon.name}` : "Leave a Review — Zaloon",
  };
}
        ...
        <div className="max-w-lg mx-auto px-4 py-6 flex flex-col items-center text-center">
          {salon.logo ? (
            ...
            <img
              src={salon.logo}
              alt={salon.name}
              className="w-14 h-14 rounded-xl object-cover shadow-sm mb-3"
            />
          ) : (
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-rose-400 to-rose-600 flex items-center justify-center text-white font-bold text-xl shadow-sm shadow-rose-200 mb-3 select-none">
