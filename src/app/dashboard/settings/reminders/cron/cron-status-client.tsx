...
import { ArrowLeft, Clock, Play, Copy, Check, RefreshCw, Settings } from "lucide-react";
...
interface CronStatusClientProps {
  pendingCount: number;
}

export function CronStatusClient({ pendingCount }: CronStatusClientProps) {
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between mb-3">
          ...
          >
            ...
            Back to Reminders &amp; Notifications
          </Link>
          <Link
            href="/dashboard/settings/reminders"
            className="inline-flex items-center gap-1.5 text-xs font-medium text-violet-400 hover:text-violet-300 border border-violet-500/30 rounded-lg px-3 py-1.5 transition-colors"
          >
      ...
      <div className="rounded-xl border border-border bg-card p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Scheduled reminders waiting to be sent
          </p>
        </div>
        <span
          className={`text-2xl font-bold tabular-nums ${
            pendingCount > 0 ? "text-violet-400" : "text-muted-foreground"
          }`}
        >
          {pendingCount}
        </span>
